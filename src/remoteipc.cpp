...
namespace Dbixwall {
    ...
    RemoteIPC::RemoteIPC(GdbixLog& gdbixLog, const QString &remotePath) :
        DbixIPC(gdbixLog),
        fWebSocket("http://localhost"), fRemotePath(remotePath), fReceivedMessage()
    {
        ...
        fIsThinClient = settings.value("gdbix/thinclient", true).toBool();
    }
    ...
    {
        if ( !fIsThinClient ) {
            return DbixIPC::getLogs(addresses, topics, fromBlock);
        }
    ...
    bool RemoteIPC::endpointWritable()
    {
        ...
        return DbixIPC::endpointWritable();
    }
    ...
    qint64 RemoteIPC::endpointWrite(const QByteArray &data)
    {
        ...
        return DbixIPC::endpointWrite(data);
    }
    ...
    const QByteArray RemoteIPC::endpointRead()
    {
        ...
        return DbixIPC::endpointRead();
    }

    const QStringList RemoteIPC::buildGdbixArgs()
    {
        QStringList args = DbixIPC::buildGdbixArgs();
        if ( fIsThinClient ) {
    ...
    bool RemoteIPC::isRemoteRequest() const
    {
        ...
        switch ( fActiveRequest.getType() ) {
            ...
            case GetPeerCount: return false; // only "dbix" available
